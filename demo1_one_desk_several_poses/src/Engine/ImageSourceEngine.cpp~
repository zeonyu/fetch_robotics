...
void ImageFileReader::loadIntoCache(void)
{
	...
	if (currentFrameNo > 0) 
        {
		int tmp_FrameNo=currentFrameNo+1;
		sprintf(str_test, rgbImageMask, tmp_FrameNo);
		if (!ReadImageFromFile(cached_rgb, str_test)) 
		{
			--currentFrameNo;
			cachedFrameNo = currentFrameNo;
		}else 
		{
			int rmFileNo=currentFrameNo-1;
			sprintf(str_test, rgbImageMask, rmFileNo);
			remove(str_test);
			sprintf(str_test, depthImageMask, rmFileNo);
			remove(str_test);
		}
        }
